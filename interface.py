...
layout = [[sg.Text('Euler Buckling Test Bench v0.1', justification='center', size=(50, 1), relief=sg.RELIEF_SUNKEN)],
          ...
          [sg.Button('Connect', key='CONNECT'),sg.Text('NOT CONNECTED',size=(30,1),key='CONNECT_STATUS')],
          [sg.Button('Start', key='START'), sg.Button('Stop', key='STOP'), sg.Button('Return', key='RETURN'), sg.Button('Save', key='SAVE'),sg.Button('SwitchDir', key='SWITCHDIR')]]
window = sg.Window('EulerBucklingTestBench', layout, finalize=True,font="Helvetica 12")
...
def gui_main_loop():
    ...
    while True:
        ...
        window.Element('CONNECT').Update(disabled = True if connection != None else False)
        window.Element('START').Update(disabled = True if connection == None or running else False)
        window.Element('STOP').Update(disabled = True if connection == None or not running else False)
        window.Element('RETURN').Update(disabled = True if connection == None or running or not has_moved else False)
        window.Element('SAVE').Update(disabled = True if connection == None or running or len(forces)== 0 else False)
        window.Element('SWITCHDIR').Update(disabled = True if connection == None or running else False)
        ...
        if event == sg.WIN_CLOSED or event == 'Exit':
           ...
           connection.write(b"go\n")
            ...
            if len(forces) != len(distances):
                sg.popup('saving failed due to an internal error (list lengths are not equal)') 
            else:
